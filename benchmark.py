''' 
Usage:
   benchmark --gold=GOLD_OIE --out=OUTPUT_FILE (--stanford=STANFORD_OIE | --ollie=OLLIE_OIE |--reverb=REVERB_OIE | --clausie=CLAUSIE_OIE | --openiefour=OPENIEFOUR_OIE | --props=PROPS_OIE | --neural=NEURAL_OIE)
...
Options:
  ...
  --neural=NEURAL_OIE          Read Neural format from file NEURAL_OIE
'''
import docopt
import string
import numpy as np
...
from oie_readers.opennmtReader import NeuralReader
...
if __name__ == '__main__':
    ...
    if args['--neural']:
        predicted = NeuralReader()
        predicted.read(args['--neural'])
